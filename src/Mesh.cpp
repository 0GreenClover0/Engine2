...
Mesh::Mesh(std::vector<Vertex> const& vertices, std::vector<u32> const& indices, DrawType const draw_type, std::shared_ptr<Material> const& material,
           DrawFunctionType const draw_function)
    : material(material), m_vertices(vertices), m_indices(indices), m_draw_type(draw_type), m_draw_function(draw_function)
{
}
