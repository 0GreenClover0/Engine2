...
void SceneSerializer::auto_serialize_component(YAML::Emitter& out, std::shared_ptr<Component> const& component)
{
    ...
    else if (auto const undotest = std::dynamic_pointer_cast<class UndoTest>(component); undotest != nullptr)
    {
        out << YAML::BeginMap;
        out << YAML::Key << "ComponentName" << YAML::Value << "UndoTestComponent";
        out << YAML::Key << "guid" << YAML::Value << undotest->guid;
#if EDITOR
        out << YAML::Key << "custom_name" << YAML::Value << undotest->get_custom_name();
#endif
        out << YAML::Key << "_u8" << YAML::Value << undotest->_u8;
        out << YAML::Key << "_u16" << YAML::Value << undotest->_u16;
        out << YAML::Key << "_u32" << YAML::Value << undotest->_u32;
        out << YAML::Key << "_u64" << YAML::Value << undotest->_u64;
        out << YAML::Key << "_i8" << YAML::Value << undotest->_i8;
        out << YAML::Key << "_i16" << YAML::Value << undotest->_i16;
        out << YAML::Key << "_i32" << YAML::Value << undotest->_i32;
        out << YAML::Key << "_i64" << YAML::Value << undotest->_i64;
        out << YAML::Key << "_float" << YAML::Value << undotest->_float;
        out << YAML::Key << "_double" << YAML::Value << undotest->_double;
        out << YAML::Key << "_string" << YAML::Value << undotest->_string;
        out << YAML::Key << "_bool" << YAML::Value << undotest->_bool;
        out << YAML::Key << "_vec2" << YAML::Value << undotest->_vec2;
        out << YAML::Key << "_vec3" << YAML::Value << undotest->_vec3;
        out << YAML::Key << "_vec4" << YAML::Value << undotest->_vec4;
        out << YAML::EndMap;
    }
                                                 ...
                                                 bool const first_pass)
{
...
#if EDITOR
            Editor::Editor::get_instance()->set_component_custom_name(deserialized_component->guid,
                                                                      component["custom_name"].as<std::string>());
#endif
    ...
    else if (component_name == "CurveComponent")
    {
        ...
        else
        {
            ...
            if (component["is_smooth"].IsDefined())
            {
                deserialized_component->is_smooth = component["is_smooth"].as<bool>();
            }
    ...
    else if (component_name == "UndoTestComponent")
    {
        if (first_pass)
        {
            auto const deserialized_component = UndoTest::create();
            deserialized_component->guid = component["guid"].as<std::string>();
#if EDITOR
            Editor::Editor::get_instance()->set_component_custom_name(deserialized_component->guid,
                                                                      component["custom_name"].as<std::string>());
        ...
        {
            auto const deserialized_component =
                std::dynamic_pointer_cast<class UndoTest>(get_from_pool(component["guid"].as<std::string>()));
            if (component["_u8"].IsDefined())
            {
                deserialized_component->_u8 = component["_u8"].as<u8>();
            }
            if (component["_u16"].IsDefined())
            {
                deserialized_component->_u16 = component["_u16"].as<u16>();
            }
            if (component["_u32"].IsDefined())
            {
                deserialized_component->_u32 = component["_u32"].as<u32>();
            }
            if (component["_u64"].IsDefined())
            {
                deserialized_component->_u64 = component["_u64"].as<u64>();
            }
            if (component["_i8"].IsDefined())
            {
                deserialized_component->_i8 = component["_i8"].as<i8>();
            }
            if (component["_i16"].IsDefined())
            {
                deserialized_component->_i16 = component["_i16"].as<i16>();
            }
            if (component["_i32"].IsDefined())
            {
                deserialized_component->_i32 = component["_i32"].as<i32>();
            }
            if (component["_i64"].IsDefined())
            {
                deserialized_component->_i64 = component["_i64"].as<i64>();
            }
            if (component["_float"].IsDefined())
            {
                deserialized_component->_float = component["_float"].as<float>();
            }
            if (component["_double"].IsDefined())
            {
                deserialized_component->_double = component["_double"].as<double>();
            }
            if (component["_string"].IsDefined())
            {
                deserialized_component->_string = component["_string"].as<std::string>();
            }
            if (component["_bool"].IsDefined())
            {
                deserialized_component->_bool = component["_bool"].as<bool>();
            }
            if (component["_vec2"].IsDefined())
            {
                deserialized_component->_vec2 = component["_vec2"].as<glm::vec2>();
            }
            if (component["_vec3"].IsDefined())
            {
                deserialized_component->_vec3 = component["_vec3"].as<glm::vec3>();
            }
            if (component["_vec4"].IsDefined())
            {
                deserialized_component->_vec4 = component["_vec4"].as<glm::vec4>();
            }
            deserialized_entity->add_component(deserialized_component);
            deserialized_component->reprepare();
        }
